...
def translate_text_robust(text: str, target_lang: str) -> str:
    ...
    # Attempt 3: Retry without quotes
    try:
    ...
    print(f"❌ Fallback to original text for: '{text[:30]}...'")
    return text
...
def transcribe_and_translate(audio_path: str, target_lang: str = "hi") -> list[dict]:
    ...
    # Merge adjacent micro-segments (< 0.5s) for natural sentences
    merged = [raw_list[0]]
    for s in raw_list[1:]:
        prev = merged[-1]
...
def render_dubbed_video(video_path: str, segments: list[dict], output_file: str):
    print("🎬 Aligning timeline and compiling non-overlapping audio master...")
    orig_total_dur = get_duration(video_path)

    # Enforce strict zero-overlap sequential audio placement
    timeline_cursor = 0.0
    for seg in segments:
    ...
    # Compile master audio with pydub
    ...
    # Stretch factor for smooth pacing
    stretch_factor = final_audio_dur / orig_total_dur if orig_total_dur > 0 else 1.0
    print(f"🎬 Video stretch factor: {stretch_factor:.4f} ({orig_total_dur:.1f}s -> {final_audio_dur:.1f}s)")
